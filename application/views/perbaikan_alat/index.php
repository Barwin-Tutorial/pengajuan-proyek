        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="tbl_perbaikan_alat" class="table table-bordered table-striped table-hover nowrap">
                            <thead>
                                <tr class="bg-purple">
                                    <!-- <th>Nama Peminjam</th> -->
                                    <!-- <th>Jabatan</th> -->
...
$(document).ready(function() {
    ...
    //datatables
    table =$("#tbl_perbaikan_alat").DataTable({
        "responsive": true,
        "autoWidth": false,
        "language": {
            "sEmptyTable": "Data Perbaikan Belum Ada"
        },
        ...
        "ajax": {
            "url": "<?php echo site_url('perbaikan_alat/ajax_list')?>",
            "type": "POST"
        },
...
function hapus(id){
    ...
    if (result.value) {
        $.ajax({
            url:"<?php echo site_url('perbaikan_alat/delete');?>",
            type:"POST",
            data:"id="+id,
            cache:false,
            dataType: 'json',
...
function add()
{
    ...
    $('.modal-title').text('Perbaikan Alat'); // Set Title to Bootstrap modal title
}
...
function edit(id){
    ...
    $.ajax({
        url : "<?php echo site_url('perbaikan_alat/edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

            $('[name="id"]').val(data.id_perbaikan_alat);
            ...
            var foto = "<?php echo base_url('assets/foto/perbaikan_alat/')?>"+data.foto;
             $("#v_image").attr("src",foto);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Perbaikan Alat'); // Set title to Bootstrap modal title
            setTimeout(function() { $('input[name="scanbar"]').focus() }, 2000);
            $.ajax({
                url : 'perbaikan_alat/get_alat_by_id/',
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('perbaikan_alat/insert')?>";
    } else {
        url = "<?php echo site_url('perbaikan_alat/update')?>";
    }
...
$(document).ready(function(){
   ...
   $( "#nama_alat").autocomplete({
    source: 'perbaikan_alat/get_alat/?', 
   ...
   $( "#scanbar").change(function () {
    ...
    $.ajax({
        url : 'perbaikan_alat/get_alat_bar/',
                ...
                <form action="#" id="form" class="form-horizontal" >
                   ...
                   <!--  <div class="form-group row ">
                    ...
                    <div class="form-group row ">
                        ...
                        <div class="form-group row ">
                            <label for="nama" class="col-sm-3 col-form-label">Tanggal Masuk</label>
                            <div class="col-sm-9 kosong">
                       ...
                       <div class="form-group row ">
                            <label for="nama" class="col-sm-3 col-form-label">Tanggal Keluar</label>
                            <div class="col-sm-9 kosong">
