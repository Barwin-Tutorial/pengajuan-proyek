 ...
 * Youtube : Aryo Coding
 */
class Mod_retur_keluar extends CI_Model
{
	var $table = 'retur_keluar';
		...
		private function _get_datatables_query()
	{
		...
		$this->db->select('a.*,b.jumlah,b.id_kemasan,b.ed,c.nama as nama_barang, d.nama as nama_pelanggan, e.nama as nama_satuan');
		$this->db->join('retur_keluar_detail b', 'a.id=b.id_retur_keluar');
		$this->db->join('barang c', 'b.id_barang=c.id');
		$this->db->join('pelanggan d', 'a.id_pelanggan=d.id');
		$this->db->join('satuan e', 'b.id_kemasan=e.id');
		$this->db->from('retur_keluar a');
		$i = 0;
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('retur_keluar', $data);
    }

         function update_detail($id, $data)
    {
        $this->db->where('id', $id);
        $this->db->update('retur_keluar_detail', $data);
    }
    ...
    {   
        ...
        $this->db->select('a.*, d.nama as nama_pelanggan,d.alamat');
		$this->db->join('pelanggan d', 'a.id_pelanggan=d.id');
        return $this->db->get('retur_keluar a')->row();
    }
    ...
    {   
    	...
    	if ($id==0) {
    		$this->db->where('a.id_retur_keluar', $id);
    ...
    {   
        ...
        return $this->db->get('retur_keluar_detail')->row();
    }
